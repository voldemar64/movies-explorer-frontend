  ...
  return(
    <li className="card" key={key}>
      ...
      {pathName.pathname === '/movies' ?
        <button type="button" onClick={() => {savedMovies.some((m) => m.movieId === card.movieId) ?
          onDelete(card) :
          onSave(card)
        }}
          className={`card__like${savedMovies.some((m) => m.movieId === card.movieId) ? ' card__like_type_liked' : ''}`}></button>
        :
